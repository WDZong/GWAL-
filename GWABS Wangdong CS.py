...
data=Zero_flow.groupby('arc').get_group('node_10646-to-node_10228')['flow']
list1 = sorted(data)
Qlist=[]
for n in [30,50,70,95]:
    Q = Qn(list1,n)
    Qlist.append(Q)
Q30 = Qlist[0]
Q50 = Qlist[1]
...
def decorate_abstraction_hof(hof_arc, abstraction_arc, method):
    def wrapper():
        if hof_arc.flow_in < Q95:
            for arc in abstraction_arcs:
                my_model.arcs[arc].capacity = my_model.arcs[arc].default_capacity*0.7
        elif Q95<=hof_arc.flow_in<Q70:
            for arc in abstraction_arcs:
                my_model.arcs[arc].capacity =  my_model.arcs[arc].default_capacity*0.9
        elif Q70<=hof_arc.flow_in<=Q50:
            for arc in abstraction_arcs:
                my_model.arcs[arc].capacity =  my_model.arcs[arc].default_capacity*1.1
        elif Q50<hof_arc.flow_in<=Q30:
            for arc in abstraction_arcs:
                my_model.arcs[arc].capacity =  my_model.arcs[arc].default_capacity*1.3
        else:
            for arc in abstraction_arcs:
                my_model.arcs[arc].capacity = my_model.arcs[arc].default_capacity*1.5
        return method()
    return wrapper

hof_arc = my_model.arcs['node_10646-to-node_10228']
for arc in abstraction_arcs:
    my_model.arcs[arc].default_capacity = my_model.arcs[arc].capacity
    
river_node = my_model.nodes['815-river']
river_node.distribute = decorate_abstraction_hof(hof_arc, abstraction_arcs, river_node.distribute)
...
import pickle
filepath = os.path.join(results_dir, "tanks-235.pkl")
with open(filepath, 'wb') as fp:
    pickle.dump(tanks, fp)
filepath = os.path.join(results_dir, "flows-235.pkl")
with open(filepath, 'wb') as fp:
